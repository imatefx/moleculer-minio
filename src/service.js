...
module.exports = {
	...
	actions: {
		...
		makeBucket: {
			...
			handler(ctx) {
				return this.Promise.resolve(ctx.params)
					.then(({bucketName, region = ""}) => this.client.makeBucket(bucketName, region));
			}
		},
		/**
         * Lists all buckets.
		...
		listBuckets: {
			handler() {
				return this.client.listBuckets().then(buckets => isUndefined(buckets) ? [] : buckets);
			}
		},
		/**
		...
		listObjects: {
			...
			handler(ctx) {
				return this.Promise.resolve(ctx.params)
					.then(({bucketName, prefix = "", recursive = false}) => {
		...
		listObjectsV2: {
			...
			handler(ctx) {
				return this.Promise.resolve(ctx.params)
					.then(({bucketName, prefix = "", recursive = false, startAfter = ""}) => {
						return new this.Promise((resolve, reject) => {
							try {
								const stream = this.client.listObjectsV2(bucketName, prefix, recursive, startAfter);
								const objects = [];
								stream.on("data", el => objects.push(el));
								stream.on("end", () => resolve(objects));
								stream.on("error", reject);
		...
		getPartialObject: {
			...
			handler(ctx) {
				return this.client.getPartialObject(ctx.params.bucketName, ctx.params.objectName, ctx.params.offset, ctx.params.length);
			}
		},
		/**
		...
		fGetObject: {
			...
			handler(ctx) {
				return this.client.fGetObject(ctx.params.bucketName, ctx.params.objectName, ctx.params.filePath);
			}
		},
		/**
		...
		putObject: {
			handler(ctx) {
				return this.Promise.resolve({stream: ctx.params, meta: ctx.meta})
					.then(({stream, meta}) => this.client.putObject(meta.bucketName, meta.objectName, stream, meta.size, meta.metaData));
			}
		},
		/**
		...
		copyObject: {
			...
			handler(ctx) {
				return this.Promise.resolve(ctx.params)
					.then(({bucketName, objectName, sourceObject, conditions}) => {
						...
						return this.client.copyObject(bucketName, objectName, sourceObject, conditions);
					});
			}
		},
		/**
		...
		removeIncompleteUpload: {
			...
			handler(ctx) {
				return this.Promise.resolve(ctx.params)
					.then(({bucketName, objectName}) => this.client.removeIncompleteUpload(bucketName, objectName));
			}
		},
		/**
				...
				return this.Promise.resolve(ctx.params)
					.then(({policy}) => {
						...
						if (policy.contentLengthRangeMin && policy.contentLengthRangeMax) {
							_policy.setContentLengthRange(policy.contentLengthRangeMin, policy.contentLengthRangeMax);
						}
